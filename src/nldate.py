...
def parse(s: str, today: date | None = None) -> date:
    ...
    for i in range(len(split_str)):
        ...
        if curr_word == "next":
            if next_word in weekdays.keys():
                return ref_date + timedelta(days=7 + weekdays[next_word] - ref_date.weekday())
    ...
    if increase_from_ref:
        return ref_date + timedelta(days=days_to_shift) + timedelta(weeks=weeks_to_shift) \
               + relativedelta(months=months_to_shift) + relativedelta(years=years_to_shift)
